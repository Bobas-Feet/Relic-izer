...
Opening_Message = ("                                               --------------- Welcome to the Relic-izer 3000! ---------------\n"
                    "Do you want to know how much Signal data and Salvage you're going to need to get from your"
                    " Current Relic to your desired, Target Relic? Well then, go ahead.\nType in your starting "
                    "(Current Relic) and end (Target Relic) points, click Add to queue, then Calculate to see the results.\n"
                    "\nDo you want to look at what character applies which effect? We've got you covered there as well.\n"
                    "Search for the effect you looking for in the Status Effects and Abilities section.")
...
def print_output(text, replace_top=False, text_output=None, append_default=False, clear=False):
    if not text_output:
        return
    ...
    if clear:
        ...
        text_output.insert(ctk.END, full_text + "\n\n")
        text_output.see(ctk.END)  # Scroll to bottom to show latest message
...
def highlight_frame(frame, status):
    ...
    frame.highlight_status = status  # track current state
    frame.configure(
        border_color=colors.get(status, "gray"),
...
def clear_all(current_entry=None, target_entry=None, name_entry=None, status_effect_combobox=None,
              text_output=None, calculation_history=None, current_frame=None, target_frame=None):
    ...
    if confirm:
        ...
        if status_effect_combobox is not None:
            status_effect_combobox.delete(0, "end")
        calculation_history.clear()
        reset_field_styles(current_frame, target_frame)
        print_output(Opening_Message, text_output=text_output, clear=True)
...
def add_calculation(text_output=None, current_entry=None, target_entry=None, current_frame=None,
    ...
    try:
        ...
        if not is_valid:
            print_output("What the hell is this? Sith have red frames. Make sure yours is green to continue.", text_output=text_output, clear=True)
            return
        ...
        print_output(msg, text_output=text_output, replace_top=not name, clear=True)
...
def summarize_all(calculation_history=None, text_output=None,
    ...
    if not calculation_history:
        ...
        print_output(
            ...
            "click Add to Queue, then Calculate.", text_output=text_output, clear=True)
        return
    ...
    if not is_valid:
        highlight_frame(current_frame, "invalid")
        highlight_frame(target_frame, "invalid")
        print_output("Something's missing. Either it's Current Relic, Target Relic, or both.", text_output=text_output, clear=True)
        return
    ...
    print_output("\n".join(output_lines), text_output=text_output, clear=True)
